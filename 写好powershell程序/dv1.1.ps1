...
function Install-Spexe($spNumber,$spName,$line){
 ...
 function Install-Spexe($spNumber,$spName,$line){
    ...
    while (1){
        ...
        else {write-host -f 13 "Msg: Not found runpycha.exe !";Start-Process ".\Au3Install";read-host;}}
    writeStateLog "compare" "$line"
...
Function Get-SoftReg {
    ...
    $UninstallPaths |foreach {
        Get-ChildItem $_ |foreach {
        Get-ItemProperty $_.pspath |where displayname -like "*$($SoftName)*"
        }
    }
}
Get-SoftReg "Intel(R) Management Engine Interface"
#>